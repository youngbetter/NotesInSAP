...
include_recipe 'jenkins-server::ant'
include_recipe 'jenkins-server::maven'
include_recipe 'jenkins-server::jvm7'
include_recipe 'jenkins-server::jvm8'
include_recipe 'jenkins-server::git'
include_recipe 'jenkins-server::jenkins'
include_recipe 'jenkins-server::nvm'
...
echo 'SELECT * FROM pg_database;' 
su - #{node['private_chef']['postgresql']['username']} 
-c '/opt/opscode/embedded/bin/psql -p #{node['private_chef']['postgresql']['port']} -U #{node['private_chef']['postgresql']['db_superuser']} postgres -t -A'

7991941dedb740328d24af0ea6659dc4